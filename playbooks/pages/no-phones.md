...
- **Send a runner in a pair**, with a written note: who, what, where (a grid reference from the map), when, how many. A bicycle beats a car in a blocked town.
- **Flag down** a police car, ambulance, fire engine, or anything with a radio: buses, utility vans, taxis, delivery drivers, farmers and fishermen often have radios that still work.
- **Rest centres and wardens**: councils open rest centres in schools and halls in a declared emergency, and they have radios to the control room. The council's emergency planners run them ([Community module](module:community)).
...
For a medical emergency, the quick cards are written to be used without a phone: [CPR](card:cpr-adult), [severe bleeding](card:severe-bleeding), [choking](card:choking), [the recovery position](card:recovery-position). Treat, then move the patient to help rather than waiting for help to come, using a car, a barrow, a stretcher of poles and jackets ([Rope, knots and tools](page:fieldcraft-rope-tools)). For fire, the household plan's meeting point and a bucket chain; for flood, higher ground; for a missing person, the search method on [Getting found](page:fieldcraft-rescue).
