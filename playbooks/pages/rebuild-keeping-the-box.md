...
The official supply is a 27 W USB-C unit, which delivers 25.5 W to the Pi at 5.1 V — headroom, not consumption. The Pi's own typical figure is about 4 W, with peaks around 12 W, and the official 7-inch display adds about 1 W at full brightness. Work from the runtime as well: a 20,000 mAh USB-C power bank runs the box for roughly 8 to 15 hours, the longer figure with the screen off ([About Operation SOS](page:about-sos); [What still works](page:what-still-works)). A 20,000 mAh cell at its nominal 3.7 volts holds about 74 watt-hours, of which perhaps 45 to 60 survive the conversion to 5 volts. Put the two together and call it 4 to 5 watts dark and 5 to 6 lit, plus whatever the drive draws. That is arithmetic from published figures rather than a measurement, but it is the right order: this box costs about as much power as a bright LED bulb, and a 250 Wh portable battery station runs it for about two days on its DC or USB-C output. Run the same load through the station's AC inverter instead and you pay both the conversion loss and the inverter's own constant overhead, which is significant against a 5 W load.

That changes what it takes to keep it alive:

- **A panel and a battery, sized for a lamp rather than a house.** UK panels average a capacity factor of about 10 per cent over the year, so a 100 W panel averages around 10 W, and London gets about 0.6 kWh of sunshine per square metre a day in December against 5.1 in July ([Solar panels in a power cut](page:solar-islanding)). A single 100 W folding panel with a charge controller therefore covers this box comfortably in summer and only on the brighter winter days, so the battery, not the panel, is what carries it through a dull week ([Photovoltaic system](kiwix:wikipedia_en_all_maxi/Photovoltaic_system); [Maximum power point tracking](kiwix:wikipedia_en_all_maxi/Maximum_power_point_tracking)).
- **Store in LiFePO4 if you have the choice.** A 12.8 V 100 Ah lithium iron phosphate battery holds about 1,280 Wh and tolerates roughly 2,500 cycles at 80 per cent depth of discharge and about 5,000 at 50 per cent, so gentler cycling lasts longer; that is weeks of this box per charge and years of cycling; a car starter battery is not built for deep discharge and a full discharge shortens its life ([Solar panels in a power cut](page:solar-islanding); [Power module](module:power)).
...
This box boots and reads from an NVMe solid-state drive, not a memory card, which is deliberate: cards are a known wear-out item in a Raspberry Pi, and Raspberry Pi's own guidance recommends replacing them on a schedule rather than waiting for one to fail. Both store data as trapped electrical charge, and that has two consequences worth knowing.
...
**An unpowered drive is not an archive.** The stored charge leaks away over years through imperfect insulation, faster when it is warm ([Data degradation](kiwix:wikipedia_en_all_maxi/Data_degradation); [Flash memory](kiwix:wikipedia_en_all_maxi/Flash_memory)). The industry retention specification is one year at 30 °C for a worn-out consumer drive, and it roughly halves for every 5 °C warmer, so a drive in a warm drawer is a copy with an expiry date nobody can read. Power every spare drive up at least once a year — sooner if it was heavily written — read the whole of it back, and copy it afresh onto another drive every few years. Store drives cool and dry, in a sealed bag with a desiccant sachet, out of the sun.
...
The reason for the tin is electromagnetic pulse, and it needs stating honestly. A Faraday cage is a continuous conductive covering or mesh that blocks *some* electromagnetic fields — not steady or slowly varying magnetic ones, and only where the gaps in it are small compared with the wavelength ([Faraday cage](kiwix:wikipedia_en_all_maxi/Faraday_cage); [Electromagnetic pulse](kiwix:wikipedia_en_all_maxi/Electromagnetic_pulse)). A biscuit tin is not a laboratory enclosure, and the published guidance is blunt about it: metal bins and tins do not usually give reliable protection unless they have been modified to block radio waves getting in through the gaps at the lid, the handles and sometimes the base. The same guidance notes that small handheld electronics are relatively immune anyway unless they have long antennas or power cords attached, and that the practical fix is to wrap each item in insulating material and then in two or more complete, overlapping layers of heavy-duty foil before it goes in the tin ([Power module](module:power)). The tin costs nothing, it keeps out damp, mice and casual borrowing, and what was switched off and unplugged is the likelier survivor in any case.
...
cd /srv/sos && find . -type f -exec sha256sum {} + > /mnt/spare/checksums.txt
cd /mnt/spare/sos && sha256sum -c ../checksums.txt
```

Generate the checksum file from the original and run the check from the root of the copy: the paths stored in it are relative, so checking from anywhere else fails every line. On a local drive-to-drive copy `rsync` sends whole files by default, which is why `--partial-dir` rather than plain `--partial` is what actually lets an interrupted run pick up where it stopped.
