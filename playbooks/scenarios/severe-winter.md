...
- **Pipes.** {{#if water}}Keep a trickle running in the coldest pipe, lag what you can reach, and know the stopcock{{else}}With the mains off or frozen, open the taps so a freezing pipe has somewhere to expand, lag what you can reach, and know the stopcock{{/if}}; a burst when it thaws floods the house ([Tools and repair module](module:tools-repair)).
