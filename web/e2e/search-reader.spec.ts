...
test('Now -> Find -> search -> open an article in the reader', async ({ page }) => {
  ...
  // One list, by relevance: the box's own and the library's together.
  const library = page.getByRole('region', { name: 'Results' });
...
test('a search for what is meant marks the rows it found by meaning "related", and the books are their own source', async ({ page }) => {
  ...
  const own = page.getByRole('region', { name: 'Results' });
  const library = own;
  await expect(own.getByRole('listitem')).toHaveCount(5);
  await expect(page.getByText('5 results')).toBeVisible();

  // "related" is on the rows found by meaning, the box's and the library's, and only on those.
  ...
  await expect(library.locator('.result-source', { hasText: 'Page' })).toHaveCount(0);
