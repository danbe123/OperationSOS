...
const SHOTS: Shot[] = [
  { name: 'now-peacetime', go: async (p, s) => { s.household = [{ id: 1, name: 'Sam', age: 41, needs: '', medications: '', contacts: '', updated_at: hour() }, { id: 2, name: 'Alex', age: 12, needs: 'asthma', medications: 'salbutamol', contacts: '', updated_at: hour() }]; s.stock = [{ id: 1, name: 'Bottled water', category: 'water', quantity: 27, unit: 'L', per_person_day: 3, expires: null, notes: '', updated_at: hour(), days_left: 4.5, kit_item: null }]; await p.goto('/'); await expect(p.getByRole('region', { name: 'How ready you are', exact: true })).toBeVisible(); } },
  ...
  { name: 'stock', go: async (p, s) => { s.stock = [{ id: 1, name: 'Bottled water', category: 'water', quantity: 13.5, unit: 'L', per_person_day: 3, expires: null, notes: '', updated_at: hour(), days_left: 1.5, kit_item: null }]; await p.goto('/plan#stock'); await p.getByRole('region', { name: 'Stock' }).scrollIntoViewIfNeeded(); } },
