...
const ONLY = process.env.SOS_SHOTS_ONLY ?? '';
const SIZES = [
...
const SHOTS: Shot[] = [
  ...
  { name: 'tasks', go: async (p, s) => { off(s, 'power'); await p.goto('/tasks'); await expect(p.getByRole('heading', { level: 1, name: 'Tasks' })).toBeVisible(); } },
  ...
  { name: 'guides-filtered', go: async (p) => { await p.goto('/guides'); await p.getByRole('searchbox', { name: 'Filter the guides' }).fill('water'); await expect(p.getByRole('status')).toBeVisible(); } },
  ...
  { name: 'household', go: async (p, s) => { s.household = [{ id: 1, name: 'Sam', age: 41, needs: 'asthma', medications: 'salbutamol inhaler', contacts: 'GP 023 8000 0000', updated_at: hour() }]; await p.goto('/plan'); await expect(p.getByRole('region', { name: 'Household' })).toBeVisible(); } },
