...
const mocks = vi.hoisted(() => {
  const rendition = { display: vi.fn(async () => undefined), next: vi.fn(), prev: vi.fn(), on: vi.fn(), off: vi.fn(), themes: { register: vi.fn(), select: vi.fn(), fontSize: vi.fn() }, hooks: { content: { register: vi.fn() } }, getContents: () => [] };
