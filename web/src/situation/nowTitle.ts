...
/** Peacetime, in one line: the services are the only thing the heading is about. The box used to
 * add half a sentence about a score and a cupboard — "Not ready yet", "Nothing stored yet" — over a
 * household that had told it nothing, which was the box marking its own homework in front of them.
 * With no register and no stock there is one true thing left to say, and this says it. */
export function peacetimeTitle(): string {
...
export function nowTitle(view: SituationView | null): string {
  ...
  if (!broken.length) return peacetimeTitle();
