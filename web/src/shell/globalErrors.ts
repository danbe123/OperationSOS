 ...
 * take the screen with it. They are deliberately not `preventDefault`ed: the browser (and Playwright's
 * `pageerror`) must still hear a real uncaught error. The same message is logged only a few times, so a loop of them cannot fill the
