...
@router.get("/search")
async def do_search(request: Request, q: str = Query(default="", max_length=512),
                    sources: str | None = None, limit: int = 40, conn=Depends(get_db)):
...
@router.get("/suggest")
async def do_suggest(request: Request, q: str = Query(default="", max_length=512), conn=Depends(get_db)):
