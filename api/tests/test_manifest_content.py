...
def test_build_items_have_artifact_and_seed_lists():
    for it in items("core.json"):
        src = it["source"]
        if src["type"] == "build":
            assert src["tool"] in {"zimit", "build-nhs", "build-crawl", "manual", "build-embeddings", "pdf2epub"}, it["id"]
            if it["kind"] == "dir":                 # a directory item is unpacked in place: artifact is its dest --
                                                     # except embeddings-household, which shares embeddings-docs's
                                                     # directory and names one specific file of it as dest instead
                if src["tool"] == "build-embeddings":
