...
- `manifest/overlays.json`: `airports-military` becomes two items, `airports` (`nwr/aeroway=aerodrome`; icon `airport`; colour as today) and `military` (`nwr/military=*` plus `nwr/landuse=military`; icon `shield`; colour `#8c564b`), both pmtiles, same coverage. The build (`api/sos/mapbuild/overlays.py`), the verifier, the style layer list (`tools/map-styles/layers/overlays.json`), the describer's titles, and the three scenarios that switch the old overlay on (`invasion`, `volcanic`, `nuclear-war`: their `overlays:` lists and any `map:?overlay=` links) are updated; `sos validate-playbooks` must pass.
