...
- **mode**: interface changes: `when: {power: off, dark: true}` → `theme: mono, dim: true`; `when: {landline: off, mobile: off}` → `calls: hidden`; `when: {scenario: storms-flooding}` → `map_first: true`.
...
- **Modes**: applied in the theme provider and the chrome: mono and dim when the mode says so; call buttons and phone numbers hidden or swapped when `calls: hidden`; the map first on Home when `map_first`.
