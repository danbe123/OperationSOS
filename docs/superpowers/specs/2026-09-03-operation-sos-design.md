...
| `kiwix-serve.service` | `kiwix-serve --library /srv/sos/state/library.xml --monitorLibrary --address all --port 8090 --urlRootLocation /kiwix --nosearchbar --nolibrarybutton --blockexternal` | `Restart=always`, `RestartSec=2` (a yanked USB drive can SIGBUS it). Library file is regenerated by sos-api |
...
| `sos-kiosk.service` | `cage -- /usr/local/bin/sos-kiosk-app` | Owns tty1 itself (no getty autologin): `PAMName=login`, `TTYPath=/dev/tty1`, `Conflicts=getty@tty1.service`, `After=systemd-user-sessions.service`, `User=sos` (in groups `video`, `input`, `render`), `Restart=always`, `RestartSec=3`, `WantedBy=graphical.target`; the install script runs `systemctl set-default graphical.target`. `ExecStartPre` waits up to 60s for `http://localhost/api/status`. The wrapper script rotates the panel with `wlr-randr --output <DSI-n> --transform 90` (or 270 for the other case orientation), then keeps `chromium --kiosk --ozone-platform=wayland --force-device-scale-factor=1.5 --noerrdialogs --no-first-run --disable-session-crashed-bubble --hide-crash-restore-bubble --disable-infobars --no-default-browser-check --disable-features=Translate --password-store=basic --user-data-dir=~/.config/sos-kiosk --overscroll-history-navigation=0 http://localhost/starting` running: before every start it resets `exit_type` and `exited_cleanly` (so no restore bubble after power loss), moves a torn `Preferences` aside and deletes the `Singleton*` locks a dead Chromium left; when Chromium dies it starts it again, after a doubling pause (1 s to 16 s) if it keeps dying within 20 s, and after five quick deaths in a row it exits so systemd restarts the session. `/starting` is a static page in the web build that polls `/api/status` and replaces itself with `/?kiosk=1`. Hung-page watchdog: the kiosk's own page (never a phone's) posts `/api/kiosk/alive` every 30 s; sos-api keeps the time in memory and reports the age at the loopback-only `GET /api/kiosk/alive-age`; the wrapper polls it and, when a Chromium whose page has spoken since that Chromium started has been quiet for more than 3 minutes, stops it (SIGTERM, then SIGKILL) so the restart loop starts a fresh one. A page never heard from since start, or an API that cannot be asked, never causes a kill |
...
| `sos-embed.service` | `llama-server -m /srv/sos/core/models/embed/bge-small-en-v1.5-q8_0.gguf --embedding --pooling cls -c 512 -ub 512 -b 512 --host 127.0.0.1 --port 8091 -t 2 --no-webui` | Always on (`ConditionPathExists=` the model, so absent it fails quietly and search is the keyword search alone), `MemoryMax=600M`, `Nice=5`: a 37 MB model answering a query in 25 ms, for semantic search (2026-09-18 search design) |
...
Crash recovery (2026-09-21): caddy, sos-api, kiwix-serve, sos-embed and sos-kiosk are `Restart=always` with `StartLimitIntervalSec=0`, so an unattended box never leaves one in `failed`: `on-failure` does not restart after SIGTERM (an out-of-memory daemon, a stray `kill`), and systemd's default start limit (5 starts in 10 s) parks a unit for good after a burst of restarts. Among them the graph uses `Wants=` and `After=` only (no `Requires=`, `BindsTo=` or `PartOf=`), so restarting sos-api takes down neither Caddy nor the kiosk browser, which retries by itself. sos-api has `TimeoutStopSec=10` and uvicorn `--timeout-graceful-shutdown 5` so an open assistant stream cannot hold a restart for systemd's 90 s. `sos-llama.service` stays `Restart=no`. A crash loop backs off: the five units keep `RestartSec` (2 s; 3 s for the kiosk; 5 s for sos-embed) as the first delay and add `RestartSteps=4` with `RestartMaxDelaySec=20` (30 for sos-embed and the kiosk), systemd 254 and later (Trixie has 257), so a broken sos-api costs about 4 per cent of a core instead of a third. Measured on systemd 255: the step count is never reset by a healthy run (30 s runs still grew 2.7 s to 11 s), so the ceiling is also the longest wait after a lone crash, which is why it is kept low.
...
- Every quick card (Home, First aid, the card: the First aid tile ends the scenario grid on Now and opens the Library's medical shelf), every scenario's Right now tab, the map and the Connect a phone panel are at most two taps from Home; everything else is reachable from the search bar, which is present on every screen.
