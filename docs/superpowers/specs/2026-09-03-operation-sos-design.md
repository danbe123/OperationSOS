...
| llama-server (Gemma 4 E2B Q4_K_M, 2.9GB file, 4K context in q4_0 KV) | 3.5GB, only when AI is on; `MemoryMax=4500M` |
...
| `sos-llama.service` | `llama-server -m /srv/sos/core/models/${SOS_MODEL} --host 127.0.0.1 --port 8081 -c 4096 -t 4 -ngl 0 -fa on -ctk q4_0 -ctv q4_0 -np 1 --no-webui --reasoning-budget 0` with `EnvironmentFile=/srv/sos/state/config/ai.env` | Disabled by default; started and stopped by sos-api via `systemctl` (sudoers rule scoped to this unit). `ExecStartPre` sets the CPU governor to performance; `ExecStopPost` restores ondemand |
...
| `flood-zones` | Flood zones | EA Flood Map for Planning zones 2 and 3 (England), NRW Flood Map for Planning (Wales), SEPA river and coastal outlines (Scotland), DfI Rivers strategic flood map (NI); RoI OPW extents if downloadable; each `ogr2ogr -f FlatGeobuf -t_srs EPSG:4326` then one tippecanoe run with a layer per region | pmtiles | england, wales, scotland, ni |
| `health` | Hospitals, pharmacies, GP surgeries | `nwr/amenity=hospital,pharmacy,doctors` → `osmium export` → `ST_PointOnSurface` via ogr2ogr keeping `name, amenity, phone, opening_hours` | geojson | all |
| `fuel` | Fuel stations | `nwr/amenity=fuel`, same pattern | geojson | all |
| `water` | Reservoirs and water works | `nwr/landuse=reservoir nwr/water=reservoir nwr/man_made=water_works` | pmtiles (over 5MB) | all |
...
| `airports-military` | Airports and military bases | `nwr/aeroway=aerodrome nwr/military=*` | pmtiles (over 5MB) | all |

Any overlay whose GeoJSON exceeds 5MB is tiled with tippecanoe and declared `pmtiles`. `scenarios_on` is derived at `sos index` from playbook `overlays:` front matter; overlays.json holds the static fields. The layer panel shows "no data for this region" from `coverage`.

### Viewer

MapLibre GL JS with the `pmtiles` protocol reading from `/maps/*` via range requests. Base switch uses `map.setStyle(next, {transformStyle})` carrying overlay, contour and hillshade sources and layers across. On `EtagMismatch` the PMTiles source is re-created rather than surfacing an error. Features: layer panel with coverage notes, place search (`/api/places`), pins saved to the box (`notes` kind `pin`), measure (distance and bearing), grid reference readout for the map centre and a tapped point (proj4 with the OSGB36 `towgs84` Helmert parameters, two letters plus 6 or 8 figures; Irish Grid EPSG:29903 over NI and RoI; lat/lon only over the Channel Islands), "Locate me" on the kiosk only (phones get a note that GPS is blocked over HTTP and are offered postcode, place or grid-reference entry and the phone map packs), share-a-place as selectable text and a QR code of `/map?lat=&lon=&z=&label=`, and print (map rendered to an image with `preserveDrawingBuffer`, printed with scale bar, grid reference and legend). `/map` accepts repeatable `overlay=<id>`, which is what `map:` links expand to.
...
llama-server from a native llama.cpp build made on the Pi: `cmake -S . -B build -DGGML_NATIVE=ON -DGGML_CPU_KLEIDIAI=ON -DLLAMA_BUILD_TESTS=OFF && cmake --build build --config Release -j4` (native detection yields `armv8.2-a+dotprod+fp16`; never `+i8mm`, the Cortex-A76 lacks it). Unit flags in section 5. sos-api calls `POST /v1/chat/completions` with `stream: true`, `cache_prompt: true`, `max_tokens: 400`, `temperature: 0.2`, `chat_template_kwargs: {"enable_thinking": false}`. Readiness is `GET /health` returning 200. Expected on the Pi 5 with a tuned build: about 20 tokens per second prompt processing and 7 to 12 generation, so a 1,500-token prompt gives roughly 75 seconds to first token and a 200-token answer 20 to 30 seconds more; the UI shows progress for both phases. Concurrency: `-np 1` and a single asyncio lock in sos-api; a second question while one is running gets `error {code: "busy", retry_after}` at once.
...
6. **AI** (05). CI: the fake llama-server suite; `sos eval --retrieval-only` at least 80% on the fixture library. Box: `sos eval` full run with median time to first token at most 90 seconds, generation at least 7 tokens per second, zero swap, CPU under 80°C; results file committed.
